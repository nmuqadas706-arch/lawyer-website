<?php
  ...
  /*
   * Session Start
   * Checking if the user is already logged in to redirect them.
   */
  session_start();
   ...
   * Processes the form submission when the user attempts to log in.
   */
  $login_error = "";
  if(isset($_POST['login'])) {
      ...
      if(empty($email) || empty($password)){
          ...
          // Fetch Data: Query the database for the user
          $query = "SELECT * FROM customers WHERE email='$email_safe' AND password='$password_safe'";
          $result = mysqli_query($conn, $query);

          // Session Check: If a matching record is found, establish the session
...
<div class="auth-container">
  ...
  <div class="container">
    <div class="auth-card-luxury" data-reveal="fade">
      ...
      <form id="customerLoginForm" method="POST" action="" onsubmit="return validateCustomerLogin();">
        <!-- Email -->
        <div class="form-field-luxury">
          <label for="email">Email Address</label>
          <input type="email" name="txt_email" class="luxury-input form-control" id="email" placeholder="your@email.com" required maxlength="100" autocomplete="email">
        </div>
        ...
        <div class="form-field-luxury">
          ...
          <input type="password" name="txt_password" class="luxury-input form-control" id="password" placeholder="Enter your password" required minlength="6" maxlength="50" autocomplete="current-password">
        </div>
